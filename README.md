...
diagnosing why a specific x86 program misbehaves under it. If you're
profiling a normal Linux app, use perf or valgrind — the generic tools
work fine for native code. These tools exist because *box64* breaks the
generic tools' assumptions in specific ways:
...
Constraints worth knowing up front: Linux-only (eBPF), requires root,
`python3-bcc`, and a box64 built with debug symbols (no strip). Tied
to box64's symbol stability — see `tests/test_upstream_compat.py`.
